...
<security:authorize access="hasRole('ADMIN')">
	<display:table pagesize="5" name="quolets" id="row" requestURI="quolet/administrator/list.do">

		<display:column>
			<a href="quolet/administrator/show.do?quoletId=${row.id}"><spring:message code="quolet.show" /></a>
		</display:column>
		<jstl:if test="${ row.draftMode eq 1 }">
			<jstl:set var="css" value="BLANK"></jstl:set>
		</jstl:if>
		<jstl:if test="${row.numMonth ne null }">
			<jstl:choose>
				<jstl:when test="${row.numMonth < 1 and row.draftMode eq 0}">
					<jstl:set var="css" value="LESS_1_MONTH"></jstl:set>
				</jstl:when>

				<jstl:when
					test="${row.numMonth < 2 and row.numMonth >= 1 and row.draftMode eq 0}">
					<jstl:set var="css" value="BETWEEN_1_AND_2_MONTH"></jstl:set>
				</jstl:when>

				<jstl:when test="${row.numMonth >= 2 and row.draftMode eq 0}">
		...
		<jstl:choose>
			...
			<jstl:otherwise>
				<display:column class="${css}" property="publicationMoment"
