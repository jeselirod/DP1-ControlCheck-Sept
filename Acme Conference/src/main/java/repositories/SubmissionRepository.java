...
public interface SubmissionRepository extends JpaRepository<Submission, Integer> {
	...
	//under-reviwed
	@Query("select s from Submission s where s.conference.finalMode=1 and  s.status=0")
	public Collection<Submission> getSubmissionByAdministratorStatus0();

	//Rejected
	@Query("select s from Submission s where s.conference.finalMode=1 and s.status=1")
	public Collection<Submission> getSubmissionByAdministratorStatus1();

	//Accepted
	@Query("select s from Submission s where s.conference.finalMode=1 and s.status=2")
